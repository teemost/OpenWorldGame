  ...
  return (
    ...
    <div style={{ position:'fixed',inset:0,pointerEvents:'none',zIndex:100 }}>
      ...
      <div style={{ position:'absolute',top:14,right:14,display:'flex',flexDirection:'column',alignItems:'flex-end',gap:6 }}>
        ...
        <button
          ...
          style={{
            ...
            color:'#666',padding:'3px 10px',borderRadius:4,
      ...
      <div style={{ position:'absolute',bottom:20,left:14,display:'flex',flexDirection:'column',gap:8 }}>
        ...
        {/* Camera hint */}
        <div style={{
          background:'rgba(0,0,0,0.5)',color:'#444',padding:'3px 8px',
          borderRadius:4,fontSize:10,fontFamily:'monospace',
        }}>
          Drag right side to orbit camera
        </div>
      </div>
      ...
      {/* Admin panel toggle */}
      {isAdmin && (
        <div style={{ position:'absolute',top:14,left:'50%',transform:'translateX(-50%)',pointerEvents:'auto' }}>
          <button onClick={()=>setShowAdmin(v=>!v)} style={{
            background:'rgba(80,50,0,0.85)',border:'1px solid rgba(255,200,0,0.5)',
            color:'#FFD700',padding:'5px 16px',borderRadius:6,
    ...
    {isAdmin && showAdmin && (
      <div style={{
        ...
        width:420,maxHeight:'65vh',overflowY:'auto',zIndex:500,
        background:'rgba(10,8,0,0.96)',border:'1px solid rgba(255,200,0,0.35)',
        borderRadius:10,padding:'16px 20px',fontFamily:'monospace',
        boxShadow:'0 8px 40px rgba(0,0,0,0.8)',pointerEvents:'auto',
      }}>
        <div style={{ color:'#FFD700',fontSize:14,fontWeight:'bold',marginBottom:12,letterSpacing:2 }}>
          👑 ADMIN CONTROL PANEL
        </div>
        <div style={{ color:'#888',fontSize:11,marginBottom:14 }}>
          Logged in as: <span style={{ color:'#FFD700' }}>{username}</span>
          &nbsp;·&nbsp;Email: <span style={{ color:'#aaa' }}>{currentUser?.email}</span>
        </div>
        <div style={{ color:'#666',fontSize:11,marginBottom:8,letterSpacing:1 }}>── REGISTERED PLAYERS ──</div>
        {getAllUsers().length === 0 ? (
          <div style={{ color:'#555',fontSize:12 }}>No registered users yet.</div>
        ) : (
          getAllUsers().map(u => (
            <div key={u.id} style={{
              ...
              <div>
                ...
                <div style={{ color:'#888',fontSize:10,marginTop:2 }}>
                  Lv.{u.level} · Score:{u.totalScore.toLocaleString()} · ${u.totalMoney.toLocaleString()}
                </div>
              </div>
              <div style={{ display:'flex',gap:6 }}>
                <div style={{
                  background:u.characterColor,width:16,height:16,borderRadius:'50%',
        ...
        <div style={{ marginTop:12,color:'#555',fontSize:10,textAlign:'center' }}>
